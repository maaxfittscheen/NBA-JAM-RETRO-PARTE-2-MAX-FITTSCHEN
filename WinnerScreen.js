...
export default function WinnerScreen({ route, navigation }) {
  ...
  const topRestantes = stats
    .sort((a, b) => b.score - a.score)
    .filter(p => p.name !== mvp.name)
    .slice(0, 4);
...
const styles = StyleSheet.create({
  ...
  contentContainer: { 
    flexGrow: 1,           // <--- FUNDAMENTAL PARA EL CENTRADO VERTICAL
    alignItems: 'center', 
    justifyContent: 'center', // <--- CENTRA TODO EN EL EJE Y
    paddingVertical: 20, 
  },
  winnerBox: { 
    width: '92%', 
  ...
  statRank: { color: '#FFD700', width: 30, fontWeight: '900', fontSize: 14 },
  statName: { color: '#fff', fontSize: 11, fontWeight: 'bold' },
  statTeam: { color: '#666', fontSize: 9 },
  statPts: { color: '#00f2ff', width: 50, textAlign: 'right', fontWeight: '900', fontSize: 13 },
  rematchBtn: { 
    marginTop: 20, 
